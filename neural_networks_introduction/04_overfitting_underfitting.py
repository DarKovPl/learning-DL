...
smaller_model.add(layers.Dense(units=4, activation='relu', input_shape=(max_index + 1,)))
...
baseline_line.add(layers.Dense(units=16, activation='relu', input_shape=(max_index + 1,)))
baseline_line.add(layers.Dense(units=16, activation='relu'))
baseline_line.add(layers.Dense(units=1, activation='sigmoid'))

baseline_line.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['accuracy', 'binary_crossentropy'])
...
bigger_model.add(layers.Dense(units=512, activation='relu', input_shape=(max_index + 1,)))
...
for name, history in zip(['smaller', 'baseline', 'bigger'], [smaller_history, baseline_history, bigger_history]):
    ...
    fig.add_trace(
        go.Scatter(
            x=hist['epoch'], y=hist['binary_crossentropy'], name=f'{name}_binary_crossentropy', mode='lines+markers'
        )
    )
    fig.add_trace(
        go.Scatter(
            x=hist['epoch'], y=hist['val_binary_crossentropy'], name=f'{name}_val_binary_crossentropy',
            mode='lines+markers'
        )
    )
...
regularized_model.add(layers.Dense(
    units=64, kernel_regularizer=model_regularizer.l2(0.0001), activation='relu', input_shape=(max_index + 1,)
))
...
fig = go.Figure()
for name, history in zip(['baseline', 'regularized'], [baseline_history, regularized_history]):
    hist_reg = pd.DataFrame(history.history)
    hist_reg['epoch'] = history.epoch
    fig.add_trace(
        go.Scatter(
            x=hist_reg['epoch'], y=hist_reg['binary_crossentropy'], name=f'{name}_binary_crossentropy', mode='lines+markers'
        )
    )
    fig.add_trace(
        go.Scatter(
            x=hist_reg['epoch'], y=hist_reg['val_binary_crossentropy'], name=f'{name}_val_binary_crossentropy',
            mode='lines+markers'
        )
    )
    fig.update_layout(xaxis_title='Epochs', yaxis_title='Binary Crossentropy')
